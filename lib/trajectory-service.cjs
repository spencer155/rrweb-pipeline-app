...
async function getVideoDataFromOss({ videoAuditToken, policyUuid, log }) {
  ...
  const url = buildApiUrl(apiBase, "api/audit/getVideoDataFromOss", {
    videoAuditToken,
    userUuid,
    appId,
    policyUuid: policyUuid.trim(),
...
async function queryTrajectory({ policyUuid, log }) {
  if (!policyUuid || policyUuid.trim().length < 32) {
    throw new Error("请输入正确的 policyUuid（至少 32 位）");
  }
  ...
  const videoAuditToken = policyInfo.token;
